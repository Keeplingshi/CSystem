...
<form id="formId" action="${pageContext.request.contextPath}/instructor/class/classSearchList" method="post">
...
<script type="text/javascript">
	...
	$("#classAddButton").click(function(){
	    parent.layer.open({
	        ...
	        area : ['380px' , '300px'],
	        offset: ['150px'],
	        content: '${pageContext.request.contextPath}/instructor/class/classAdd',
	        end: function(){
				//默认加载班级列表
	    		$("#formId").ajaxSubmit(function(data){
	        	 	$("#content_page").html(data);
	...
	function viewclass(classId)
	{
	    parent.layer.open({
	        ...
	        area : ['380px' , '360px'],
	        offset: ['150px'],
	        content: '${pageContext.request.contextPath}/instructor/class/classView/'+classId
	    });
	}
	...
	function updateclass(classId)
	{
	    parent.layer.open({
	        ...
	        area : ['380px' , '300px'],
	        offset: ['150px'],
	        content: '${pageContext.request.contextPath}/instructor/class/classEdit/'+classId,
	        end: function(){
	        	//默认加载用户列表
	        	$("#formId").ajaxSubmit(function(data){
	        	 	$("#content_page").html(data);
	...
	function deleteclass(classId)
	{
		...
		layer.confirm('是否确定删除？', {
		    btn: ['确定','取消'] //按钮
	 		...
	 		//默认加载班级列表
			$.post("${pageContext.request.contextPath}/instructor/class/delete/"+classId, function(result){
				if(result=='success'){
					//默认加载班级列表
		        	$("#formId").ajaxSubmit(function(data){
		        	 	$("#content_page").html(data);
	...
	function setmonitor(classId){
	    parent.layer.open({
	        ...
	        area : ['380px' , '300px'],
	        offset: ['150px'],
	        content: '${pageContext.request.contextPath}/instructor/class/setmonitorView/'+classId,
	        end: function(){
	        	//默认加载用户列表
	        	$("#formId").ajaxSubmit(function(data){
	        	 	$("#content_page").html(data);
	...
	$("#classDeleteButton").click(function(){
		...
		layer.confirm('是否确定删除这些班级？', {
			...
			console.info("确定");
			$.ajax({
				url : "${pageContext.request.contextPath}/instructor/class/deleteClasses",
				async: false,
				data : {
					"classIds" : classIds
				},
