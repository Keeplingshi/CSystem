 ...
 */
 (function($){
	...
	// Extend jQuery
	$.fn.pagination5 = function(maxentries, opts){
		...
		function InitTable(pageIndex) { 
			$.ajax({ 
				type: "POST", 
				dataType: "text", 
				url:"pagingDetention.do?flag=police", //提交到一般处理程序请求数据 
				...
				success: function(data) { 
					...
					var result_list = eval(data);
					  $.each(result_list, function (index, item) {  
							...
							var smallText = null;
							if(reason.length>10){
								smallText = reason.substring(0,10)+"……";
							}else{
								smallText = reason;
							}
							var reasonTdText = document.createTextNode(smallText);
