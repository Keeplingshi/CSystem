 ...
 */
 (function($){
	...
	// Extend jQuery
	$.fn.pagination6 = function(maxentries, opts){
		...
		function InitTable(pageIndex) { 
			$.ajax({ 
				type: "POST", 
				dataType: "text", 
				url:"pagingArrest.do?flag=police", //提交到一般处理程序请求数据 
				data:"pageIndex=" + (pageIndex)+ "&pageSize=" + 6,/*"pageIndex=" + (pageIndex) + "&pageSize=" + pageSize*/ //提交两个参数：pageIndex(页面索引)，pageSize(显示条数) 
				success: function(data) { 
					/*$("#table_1 tr:gt(0)").remove(); //移除Id为Result的表格里的行，从第二行开始（这里根据页面布局不同页变） 
					$("#table_1").append(data); //将返回的数据追加到表格*/
				     ...
				     $.each(result_list, function (index, item) {  
				         //循环获取数据    
				    	   var id = result_list[index].id;  
							...
							var reasonTdText = document.createTextNode(smallContent);
							reasonTd.appendChild(reasonTdText);
							...
							if(result_state == false||state == false){
								...
								aDeleteElement.setAttribute("href", "deletArrest.do?id="+caseRegisterId);
								var deleteText = document.createTextNode("删除 |  ");
								aDeleteElement.appendChild(deleteText);
								var aEditElement = document.createElement("a");
								aEditElement.setAttribute("href", "editArrest.do?id="+caseRegisterId);
