 ...
 */
 (function($){
	...
	// Extend jQuery
	$.fn.adminpagination2 = function(maxentries, opts){
		opts = $.extend({
		...
		function InitTable(pageIndex) { 
			document.getElementById("nowPage").innerText = pageIndex+1;
			$.ajax({ 
				type: "POST", 
				dataType: "text", 
				url:"pagingLD.do?flag=admin", //提交到一般处理程序请求数据 
					...
					var result_list = eval(data);
				     $.each(result_list, function (index, item) {  
						...
						var operationTd = document.createElement("td");
			    			...
			    			aAgreeElement.setAttribute("href", "adminhandlerLD.do?flag="+true+"&id="+caseRegisterId);
			    			...
			    			aDisAgreeElement.setAttribute("href", "adminhandlerLD.do?flag="+false+"&id="+caseRegisterId);
