...
<script type="text/javascript">
	...
	${r"$"}(function() {
		...
		${r"$"}('#form').form({
			url : '${r"$"}{pageContext.request.contextPath}/${obj}/update',
			onSubmit : function() {
				//editor.sync();
...
<div class="easyui-layout" data-options="fit:true,border:false">
	<div data-options="region:'center',border:false" title="" style="overflow: hidden;">
		...
		<input type="hidden" id="id" name="id" value="${r"$"}{codeBook.id}" >
			<table class="table table-hover table-condensed">
			<#list columnList as l>
				<tr>
					<th>${l.remarks}:</th>
					<td><input name="${l.name}" type="text" placeholder="请输入" class="easyui-validatebox span2" data-options="required:true" value="${r"$"}{${obj}.${l.name}}"></td>
				</tr>
			</#list>
			</table>
		</form>
